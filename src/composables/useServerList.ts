...
import { getCountries, getNews, getServers, getVersion } from '../services/bpserversApi'
...
export function useServerList() {
  ...
  const filteredServers = computed(() => {
    const filtered = servers.value.filter((server) => {
      ...
      if (filters.value.whitelist !== 'any' && server.Whitelist !== whitelist) return false
      if (filters.value.validation !== 'any' && server.Validation !== validation) return false
      if (
      ...
      if (filters.value.hideFull && server.PlayerCount >= server.PlayerLimit) return false
      return true
    })
  ...
  const playerCount = computed(() =>
    filteredServers.value.reduce((total, server) => total + server.PlayerCount, 0),
  )
  ...
  function loadFilters() {
    ...
    try {
      filters.value = { ...defaultFilters, ...(JSON.parse(savedFilters) as Partial<Filters>) }
    } catch {
